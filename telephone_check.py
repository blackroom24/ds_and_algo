...
it has the format of a valid US number. The following are examples of valid formats for US numbers
...
provided, you must confirm that the country code is 1. Return true if the string is a valid
US phone number; otherwise return false.
"""
import re


def telephone_check(tele_no):
    pattern = r"^(1\s?)?(\(\d{3}\)|\d{3})[\s\-]?\d{3}[\s\-]?\d{4}$"
    if re.match(pattern, tele_no):
        return True
    return False


print(telephone_check("1 555-555-5555"))
